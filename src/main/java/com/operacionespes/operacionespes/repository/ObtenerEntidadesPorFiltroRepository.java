...
public interface ObtenerEntidadesPorFiltroRepository extends JpaRepository<RelacionesArbol, Integer> {
    ...
    @Query(value = "SELECT ArbolId, pm.PersonaMoralId, LTRIM(RTRIM(pm.PersonaMoralRazonSocial)) AS RazonSocial, " +
            ...
            "    AND " +
            "    (pm.PersonaMoralId = :PersonaMoralId OR ss.SubSectorId = :SubSectorId OR ss.SubSectorNombreLargo = :SubSectorNombreLargo)", nativeQuery = true)
    List<Object[]> queryObtenerEntidadesFiltro(@Param("SubSectorNombreLargo") String SubSectorNombreLargo,
